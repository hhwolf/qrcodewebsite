...
const PRICES = { card: 1900, sticker: 1500, tent: 2200, five7: 2400 }; // cents
const FORMAT_LABELS = {
...
export default async function handler(req, res) {
  ...
  if (!unit) return res.status(400).json({ error: "Unknown format" });

  const quantity = Math.min(500, Math.max(1, parseInt(qty, 10) || 1));
  const unitAmount = quantity >= BULK_MIN ? Math.round(unit * (1 - BULK_RATE)) : unit;
